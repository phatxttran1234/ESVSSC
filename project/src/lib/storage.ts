...
export const addVideoDrill = async (video: Omit<VideoDrill, 'id' | 'created_at'>): Promise<VideoDrill> => {
  ...
  try {
    await sql`
      INSERT INTO video_drills (id, title, description, video_url, tags, category, created_at)
      VALUES (${newVideo.id}, ${newVideo.title}, ${newVideo.description}, ${newVideo.video_url}, ${newVideo.tags || null}, ${newVideo.category}, ${newVideo.created_at})
    `
    return newVideo
  } catch (error) {
